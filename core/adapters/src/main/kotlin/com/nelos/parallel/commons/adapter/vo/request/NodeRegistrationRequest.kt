 ...
 *
 * `currentLoad` + `engineConfig` are populated on `info` events only; `online`
 * carries only `capabilities` / `transports` / `resourceProviders`; `offline`
 * carries only `nodeId` + `timestamp`.
 *
 * @author gpushkarev
