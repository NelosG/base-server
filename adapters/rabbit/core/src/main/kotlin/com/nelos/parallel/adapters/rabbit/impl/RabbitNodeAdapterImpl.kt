        ...
        return try {
            rabbitTemplate.connectionFactory.createConnection().use { connection ->
                connection.createChannel(false).use { channel ->
                    ...
                    // No completion signal - count is never decremented under
                    // normal flow, so await returns false at the timeout. Latch
                    // (vs Thread.sleep) gives a clean countDown() hook if we
                    // ever want early termination (e.g. an expected reply count).
                    val deadline = CountDownLatch(1)
                    try {
                        deadline.await(timeoutMs, TimeUnit.MILLISECONDS)
